        ...
        for region in self.ledger.GetRegions():
            ...
            for process in self.ledger.GetProcesses()+['TotalBkg']:
                ...
                for time in ['prefit','postfit']:
                    ...
                    # TESTING SR PLOTTING FOR XHY - DELETE IMMEDIATELY AFTER
    ...
    def plot_projections(self, lumiText=r'138 $fb^{-1}$ (13 TeV)', extraText='Preliminary', subtitles={}, units='GeV', regionsToGroup=[]):
        ...
        for region, group in self.df.groupby('region'):
            ...
            for logyFlag in [False, True]:
                ...
                for proj in ['prefit_projx','prefit_projy','postfit_projx','postfit_projy']:
                    for islice in range(3):
                        projn     = f'{proj}{islice}'
                        sig_projn = projn
...
def make_ax_1D(outname, binning, data, bkgs=[], signals=[], title='', subtitle='', slicetitle='',
    ...
    if 'SR_pass' not in outname:
        ...
        handles, labels = ax.get_legend_handles_labels()
        unc_idx  = labels.index('Bkg. Unc.')
        sig_idxs = [i for i in range(unc_idx+1, len(labels))] # indices of all signals in legend
        leg_order = sig_idxs + [idx for idx in range(len(labels)) if idx not in sig_idxs]
...
def make_systematic_plots(twoD, do=''):
    ...
    [OPTIONAL] A specific systematic to plot can be optionally included. If the "do" option is not
               empty, then the function will skip any parameters that do not have any part of the 
               string included in the name. This saves a lot of time if you only want to look
               at one specific parameter. 
    '''
    if do != '':
        print('[2DAlphabet.plot]-------------------------------------------------')
        print(f'        Only plotting *{do}* systematics                         ')
        print('------------------------------------------------------------------')
    for (p,r), _ in twoD.df.groupby(['process','region']):
        ...
        for axis in ['X','Y']:
            ...
            nominal_hist = convert_to_events_per_unit(nominal_hist) # convert to events/unit (normalize bins by width)
            ...
            for s in twoD.ledger.GetShapeSystematics(drop_norms=True):
                ...
                if do != '':
                    # Skip any parameter that does not have the string in its name
                    if do not in s: continue  

                up_hist = getattr(twoD.organizedHists.Get(process=p,region=r,systematic=s+'Up'),'Projection'+axis)('%s_%s_%s_%s'%(p,r,s+'Up','proj'+axis))
                up_hist = convert_to_events_per_unit(up_hist)
                ...
                down_hist = convert_to_events_per_unit(down_hist)
                ...
                # Plot on main axis ("ax")
                hep.cms.text(f"WiP",loc=1,ax=ax)
                ...
                ax.set_ylim(0., nominal.max()*1.3)
                handles, labelsproj = ax.get_legend_handles_labels()
                ytitle = 'Events / %s GeV' % get_min_bin_width(nominal_hist)
                ax.set_ylabel(ytitle)
                ax.legend(loc='best')
                # Now plot on ratio axis ("rax")
                nominal_nonzero = nominal.copy(); nominal_nonzero[nominal_nonzero==0] = 1e-2
                v_up = up/nominal_nonzero
                v_dn = down/nominal_nonzero
                rax.set_ylim([0, 2])
                if ('CMS_scale' in s) or ('CMS_res' in s): # NOT GENERIC 
                    hep.histplot(v_up, edges, histtype="errorbar", label=r'$+1\sigma$/nom', color='red', ax=rax)
                    hep.histplot(v_dn, edges, histtype="errorbar", label=r'$-1\sigma/nom$', color='blue', ax=rax)
                else:
                    hep.histplot(v_up, edges, histtype="step", label=r'$+1\sigma$/nom', color='red', ax=rax)
                    hep.histplot(v_dn, edges, histtype="step", label=r'$-1\sigma/nom$', color='blue', ax=rax)
                rax.legend(fontsize=13, loc='upper right')
                rax.set_ylabel("var/nom")
                axisTitle = getattr(binning,axis.lower()+'title')
                axisTitle = axisTitle.replace('#','\\')
                rax.set_xlabel(rf'${axisTitle}$')
                rax.margins(x=0); ax.margins(x=0)
                rax.grid(axis="y")
                '''
                plt.style.use([hep.style.CMS])
                ...
                ax.set_ylim(0., nominal.max()*1.3)
                handles, labelsproj = ax.get_legend_handles_labels()
                # Convert ROOT latex to regular
                axisTitle = getattr(binning,axis.lower()+'title')
                axisTitle = axisTitle.replace('#','\\')
                ax.set_xlabel(rf'${axisTitle}$')
                ytitle = 'Events / %s GeV' % get_min_bin_width(nominal_hist)
                ax.set_ylabel(ytitle)
                plt.legend(loc='best')
                '''
...
def plot_gof(tag, subtag, seed=123456, condor=False, appendName=''):
    with cd(tag+'/'+subtag):
        if condor:
            ...
            if len(glob.glob(tmpdir+f'higgsCombine_gof_toys{appendName}.GoodnessOfFit.mH120.*.root')) == 0:
                raise Exception('No files found')
            toy_limit_tree.Add(tmpdir+f'higgsCombine_gof_toys{appendName}.GoodnessOfFit.mH120.*.root') 
            
        else:
            toyOutput = ROOT.TFile.Open(f'higgsCombine_gof_toys{appendName}.GoodnessOfFit.mH120.{seed}.root'.format(seed=seed))
        ...
        gof_data_file = ROOT.TFile.Open(f'higgsCombine_gof_data{appendName}.GoodnessOfFit.mH120.root')
