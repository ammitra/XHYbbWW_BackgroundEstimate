...
def ARC_plot(hData,hMC,hTotalBkg,labelsMC,colorsMC,xlabel,outputFile,xRange=[],yRange=[],projectionText="",yRangeLog=[]):
    ...
    '''
    bkg_yields = np.array([other, tt, qcd])
    bkg_colors = ["#ffa90e","#a96b59","#3f90da"]
    bkg_labels = ['Other',r'$t\bar{t}$','QCD']
    # Organize signal
    sig_yields = xy
    sig_colors = "#bd1f01"
    sig_scale  = 14 # postfit signal strength for S+B
    sig_labels = r'$X[1200]\to HY[900]\times %s$'%(sig_scale)
    # Organize data
    ...
    '''
    # Pull plot
    def calcPulls(hData,hMC,dataErrs,dataErr,uncBand,mcErr):
        pulls = []
        pull_errs = [[],[]]
        for i in range(len(hMC)):
            num = hData[i]-hMC[i]
            ...
            #err = abs((dataErr[i] - mcErr[i]) / den)
            errUp = dataErrs[0][i] / den
            errDn = dataErrs[1][i] / den
            pull_errs[0].append(errUp)
            pull_errs[1].append(errDn)
        return pulls, pull_errs
    def calcSystBand(hMC,uncBand):
        systBand = [[],[]]
        for i in range(len(hMC)):
            systBandUp = uncBand[0][i]/hMC[i]
            systBandDn = uncBand[1][i]/hMC[i]
            #systBandUp = uncBand[0][i]
            #systBandDn = uncBand[1][i]
            systBand[0].append(0+systBandUp)
            systBand[1].append(0-systBandDn)
        return systBand
    # Pull
    pulls, pull_errs = calcPulls(hData.bin_values,hTotalBkg.bin_values,hData.get_error_pairs(),hData.bin_error,hTotalBkg.get_error_pairs(),hTotalBkg.bin_error)
    rax.errorbar(x=centersData, y=pulls, xerr=xerrorsData, yerr=pull_errs, label=r'(Data-Pred.)/$\sigma_{stat}$', capsize=0, **errorbar_style)
    # Error band
    systBand = calcSystBand(hTotalBkg.bin_values,hTotalBkg.get_error_pairs())
    systBand_lower = list(systBand[0]) + [systBand[0][-1]]
    systBand_upper = list(systBand[1]) + [systBand[1][-1]]
    rax.fill_between(x=edges, y1=systBand_lower, y2=systBand_upper, label=r'$\sigma_{syst}$', step='post', **hatch_style)
    # Signal in rax 
    sig_rax = copy.deepcopy(sig_yields) * sig_scale
    sig_rax = sig_rax / hData.bin_error
    #rax.step(x=edges, y=np.hstack([sig_rax,sig_rax[-1]]), where='post', label=r'Signal/$\sigma_{stat}$', color=sig_colors, linewidth=2)
    hep.histplot(sig_rax, edges, ax=rax, label=r'Signal/$\sigma_{stat}$', histtype="step",color=sig_colors,linewidth=2)
    '''

    # Pull plot
    sigma = np.sqrt(hData.bin_error**2 + hTotalBkg.bin_error**2)
    pulls = (hData.bin_values - hTotalBkg.bin_values) / sigma
    hep.histplot(pulls, edges, yerr=np.ones_like(pulls), xerr=True, ax=rax, **DATA_STYLE, label=r"(Data - Bkg.) / $\sigma$")

    # Plot signal in rax
    #sig_rax = copy.deepcopy(sig_yields) * sig_scale
    #sig_rax = sig_rax / sigma
    #hep.histplot(sig_rax, edges, ax=rax, label=r'$X[1200]\to HY[900] / \sigma$', histtype="step", color=sig_colors, linewidth=3)
    ...
    leg = ax.legend([handles[idx] for idx in order],[labels[idx] for idx in order], loc='best',ncol=1, fontsize='medium')
    ...
    hep.cms.label(loc=2, ax=ax, label='', rlabel='', data=True)
...
def plot_projection(histos_dict, region, processes, labels_dict, colors_dict, axis="X",yRange=[],yRangeLog=[1,10**5], prefit=False):
    ...
    # Set up variables
    axis_label = "$M_{X}^{rec}$ [GeV]" if axis == "X" else "$M_{Y}^{rec}$ [GeV]"
    ...
    for ext in ['pdf','png']:
        ARC_plot(h_data, h_mc, h_bkg, labels_mc, colors_mc,axis_label,f"./plots/{region}_{file_suffix}.{ext}",xRange=x_range,yRange=yRange,projectionText=region.replace("_", " "),yRangeLog=yRangeLog)
...
if __name__ == "__main__":
    ...
    input_file = "../../fits_of_VR_only/VR_fits/NMSSM-XHY-1800-1200-0x0_area/postfitshapes_b.root"
    #input_file = '../../1800-1200_fits/NMSSM-XHY-1800-1200-SR1x0-VR1x0_area/postfitshapes_b.root'
    ...
    procs_SR_pass = procs + ['Background_0x0','data_obs','TotalBkg']
    procs_SR_fail = procs + ['Background','data_obs','TotalBkg']

    # procs_SR_pass = procs + ['Background_VR_pass_1x0','data_obs','TotalBkg']
    # procs_SR_fail = procs + ['Background_VR_fail','data_obs','TotalBkg']

    histos_dict = {}
    histos_dict['VR_fail'] = get_hists(input_file,'VR_fail',procs_SR_fail,prefit)
    histos_dict['VR_pass'] = get_hists(input_file,'VR_pass',procs_SR_pass,prefit)
    ...
    for k, v in {
        'VR_pass':procs_SR_pass,
        'VR_fail':procs_SR_fail
    }.items():
        for axis in ['X','Y']:
