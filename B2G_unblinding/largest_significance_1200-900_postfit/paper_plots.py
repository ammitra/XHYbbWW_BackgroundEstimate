...
def ARC_plot(hData,hMC,hTotalBkg,labelsMC,colorsMC,xlabel,outputFile,xRange=[],yRange=[],projectionText="",yRangeLog=[]):
    ...
    hep.histplot(sig_rax, edges, ax=rax, label=r'$X[1200]\to HY[900] / \sigma$', histtype="step", color=sig_colors, linewidth=4)
    ...
    hep.cms.label(loc=2, ax=ax, label=None, rlabel='', data=True)
...
if __name__ == "__main__":
    ...
    input_file = "./Unblinded_0x0/postfitshapes_b.root"

    proc_base = ['WJets','ZJets','NMSSM-XHY-1200-900','ttbar']
    procs = []
    for p in proc_base:
