...
for key, limit in limits.items():
    ...
    df.to_csv(f"limits/limits_{labels[key]}_COMBINED-ANALYSES.csv")
...
for key, grid in grids.items():
    ...
    colormesh(xx, yy, grid, t, "plots/limit2D_interp_{}_COMBINED-ANALYSES.pdf".format(label))
...
for key in range(5):
    ...
    val = limits[key]
    t = '%s expected exclusion limits (fb)'%('Median' if label=='50.0' else label+'%')
    scatter2d(val, t, "plots/limit2D_scatter_{}_COMBINED-ANALYSES.pdf".format(label))
